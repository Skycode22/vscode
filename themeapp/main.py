...
window.configure(borderwidth=5, relief=tk.RIDGE)
def generate_code():
...
def on_entry_keyrelease(event):
    ...
    width = max(1000, len(longest_line))
    topic_entry.config(width=width, height=num_lines)
topic_label = tk.Label(window, text="Topic:")
topic_label.grid(row=0, column=1, padx=10, pady=10, sticky='n')
topic_entry = tk.Text(window, width=80, height=1)
topic_entry.grid(row=3, column=1, padx=10, pady=10, sticky='ns')
...
language_label.grid(row=1, column=1, padx=10, pady=10, sticky='s')
language_dropdown = ttk.Combobox(window, textvariable=language, values=language_options)
language_dropdown.grid(row=2, column=1, padx=10, pady=10, sticky='s')
...
response_text.grid(row=3, column=0, padx=10, pady=10, sticky='news')
scrollbar = tk.Scrollbar(window, command=response_text.yview)
scrollbar.grid(row=4, column=1, sticky='news')
